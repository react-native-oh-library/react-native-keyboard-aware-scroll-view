...
const _KAM_HARMONY_EXTRA_BOOST: number = 200
  ...
  return class
    ...
    _updateKeyboardSpace = (frames: Object) => {
      ...
      if (this.props.enableAutomaticScroll) {
        let computedKeyboardHeight = frames.endCoordinates.height
        if (Platform.OS === 'harmony') {
          const windowHeight = Dimensions.get('window').height
          const derivedFromScreenY = windowHeight - frames.endCoordinates.screenY
          computedKeyboardHeight = Math.max(computedKeyboardHeight, derivedFromScreenY)
        }
        let keyboardSpace: number =
          computedKeyboardHeight + this.props.extraScrollHeight
        ...
        UIManager.measureInWindow(
          ...
          (x: number, y: number, width: number, height: number) => {
            ...
            if (Platform.OS === 'ios') {
              ...
              const harmonyBufferSpace = totalExtraHeight + _KAM_HARMONY_EXTRA_BOOST
              if (
                textInputBottomPosition >
                keyboardPosition - harmonyBufferSpace
              ) {
                this._scrollToFocusedInputWithNodeHandle(
                  currentlyFocusedField,
                  (this.props.extraHeight || 0) + _KAM_HARMONY_EXTRA_BOOST,
                  Math.max(this.props.keyboardOpeningTime || 0, _KAM_KEYBOARD_OPENING_TIME)
                )
              }
            } else {
